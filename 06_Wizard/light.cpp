...
CLight::CLight()
{
	m_color = D3DXCOLOR(1.0f, 1.0f, 1.0f, 1.0);		// 色
}
...
HRESULT CLight::Init(void)
{
	...
	D3DXVECTOR3 vecDir[m_MAX_LIGHT];				// 設定用方向ベクトル

	for (int nCntLight = 0; nCntLight < m_MAX_LIGHT; nCntLight++)
	{
		// ライトをクリアする
		...
		if (nCntLight == 0)
		{
			...
			// ライトの方向の設定
			vecDir[nCntLight] = D3DXVECTOR3(-0.45f, -0.58f, 0.35f);
