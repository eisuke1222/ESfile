...
void CSceneMesh::SetStart(D3DXVECTOR3 pos, D3DXVECTOR3 rot, D3DXVECTOR3 size, D3DXCOLOR col, D3DXVECTOR2 TexUV)
{
	...
	{// メッシュタイプ
	case MESHTYPE_FIELD:		// フィールド
		m_nMaxVtx = ((int)m_num.x + 1) * ((int)m_num.z + 1);							// 頂点数
		m_nMaxIdx = (((int)m_num.x + 1) * 2) * (int)m_num.z + (((int)m_num.z - 1) * 2);	// インデックス数
		m_nMaxPolygon = ((int)m_num.x * (int)m_num.z * 2) + (((int)m_num.z - 1) * 4);		// ポリゴン数

		nCount = (int)m_num.z;	// z枚数

		break;
	case MESHTYPE_WALL:			// 壁
		m_nMaxVtx = ((int)m_num.x + 1) * ((int)m_num.y + 1);							// 頂点数
		m_nMaxIdx = (((int)m_num.x + 1) * 2) * (int)m_num.y + (((int)m_num.y - 1) * 2);	// インデックス数
		m_nMaxPolygon = ((int)m_num.x * (int)m_num.y * 2) + (((int)m_num.y - 1) * 4);		// ポリゴン数
...
//=============================================================================
// メッシュ内にいるかチェック処理					(public)	*** CSceneMesh ***
//=============================================================================
